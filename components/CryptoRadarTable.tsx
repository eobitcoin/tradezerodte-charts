...
function signalPillClasses(signal: RadarCell["signal"]): string {
  if (signal === "buy") return "bg-emerald-500/15 text-emerald-700 dark:text-emerald-300 border-emerald-500/40";
  if (signal === "sell") return "bg-rose-500/15 text-rose-700 dark:text-rose-300 border-rose-500/40";
  if (signal === "neutral") return "bg-amber-500/10 text-amber-700 dark:text-amber-300 border-amber-500/30";
...
function rowAccent(allAgree: CryptoRadarRow["allAgree"]): string {
...
function fmtSignalPrice(p: number | null): string | null {
  if (p == null || !Number.isFinite(p)) return null;
  // Crypto sub-dollar: more decimals; large: fewer.
...
function CellView({ cell }: { cell: RadarCell }) {
  ...
  const priceStr = fmtSignalPrice(cell.price);
  return (
...
export default function CryptoRadarTable({
  rows,
  quotes,
  now,
}: {
  ...
  // All-agree first (buy then sell), then alpha by displayed ticker.
  const sorted = [...rows].sort((a, b) => {
    const rank = (r: CryptoRadarRow) =>
      r.allAgree === "buy" ? 0 : r.allAgree === "sell" ? 1 : 2;
    const rDiff = rank(a) - rank(b);
    if (rDiff !== 0) return rDiff;
    return displayTicker(a.ticker).localeCompare(displayTicker(b.ticker));
  });
  ...
  return (
    <div className="space-y-3">
              ...
              return (
                ...
                >
                  <td className="px-3 py-2 font-mono font-semibold">
                    ...
                    <div className="text-[10px] text-black/40 dark:text-white/40">/USDT</div>
                  </td>
                  <td className="px-3 py-2">
                    <div className="font-mono text-sm font-semibold">{fmtUsd(q?.usd ?? null)}</div>
                    <ChangePct pct={q?.change24h ?? null} />
                  </td>
                  {RADAR_TIMEFRAMES.map((tf) => (
      ...
      <div className="text-xs text-black/50 dark:text-white/50 leading-relaxed">
        <p className="mb-1"><strong>How it works:</strong> &quot;Current Price&quot; is fetched live from Coingecko at page load. The 4H/Daily/Weekly cells show the most recent TradingView signal for each timeframe. Rows where all 3 timeframes agree appear at the top.</p>
