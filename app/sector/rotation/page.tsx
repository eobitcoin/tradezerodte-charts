/**
 * /sector/rotation — Sector Rotation Detector
 *
 ...
 *
 * Moved here from /research/rotation so it sits alongside the new live
 * Bubbles surface under the Sector hub. The old path 308-redirects to
 * here so existing bookmarks + sitemap entries keep working.
 */
...
function EmptyState() {
  return (
    <div className="rounded-lg border border-black/10 dark:border-white/10 p-8 text-center space-y-3">
      ...
      <Link href="/sector" className="inline-block underline text-sm">
        ← Back to Sector Bubbles
      </Link>
    </div>
  );
}
      ...
      <ul className="space-y-1">
        {history.map((h) => {
          ...
          return (
            <li key={h.scanDay}>
              <Link
                href={`/sector/rotation?day=${h.scanDay}`}
                className={[
