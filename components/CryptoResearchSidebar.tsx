...
export type CryptoResearchSidebarItem = {
  scanDay: string;
  title: string;
  tradesCount: number;
};
...
export default function CryptoResearchSidebar({
  items,
  currentScanDay,
}: {
  items: CryptoResearchSidebarItem[];
              ...
              <>
                <div className="flex items-baseline justify-between gap-2">
                  ...
                  <span
                    ...
                    title={`${item.tradesCount} trade plan${item.tradesCount === 1 ? "" : "s"}`}
                  >
                    {item.tradesCount}
                  </span>
                </div>
            ...
            return (
              <li key={item.scanDay}>
                {active ? (
                  ...
                  <Link
                    href={`/crypto/research/${item.scanDay}`}
