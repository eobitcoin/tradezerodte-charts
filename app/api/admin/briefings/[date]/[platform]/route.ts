/**
 * POST /api/admin/briefings/[date]/[platform]
 *
 * Admin approval workflow for daily briefing videos. One endpoint handles all
 * state transitions for one (trading_day × platform) pair. The publish
 * routines pick up rows where `<platform>Status === 'approved'`.
 *
 * Actions:
 ...
 *
 * Platform: 'yt' (YouTube) or 'tt' (TikTok).
 */
import { NextResponse } from "next/server";
import { eq } from "drizzle-orm";
import { z } from "zod";
import { db } from "@/lib/db";
import { briefings, type PlatformPublishStatus } from "@/lib/db/schema";
...
export async function POST(
  ...
  const row = (
    await db.select().from(briefings).where(eq(briefings.tradingDay, date)).limit(1)
  )[0];
  if (!row) {
  ...
  if (!row.videoS3Key) {
    return NextResponse.json(
      { error: "video not yet rendered for this trading day" },
      { status: 409 },
    );
  }
  ...
  await db.update(briefings).set(update).where(eq(briefings.tradingDay, date));

  return NextResponse.json({ ok: true, action, platform, tradingDay: date });
}
